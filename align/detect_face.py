...
class Network(object):
    ...
    def load(self, data_path, session, ignore_missing=False):
        ...
        '''
        data_dict = np.load(
            data_path, encoding='latin1').item()  #pylint: disable=no-member
    ...
    def setup(self):
        (self.feed('data')  #pylint: disable=no-value-for-parameter, no-member
         ...
         .conv(2, 2, 64,
               ...
               relu=False,
               name='conv3').prelu(name='prelu3')
         .fc(128, relu=False, name='conv4').prelu(name='prelu4')
         .fc(2, relu=False, name='conv5-1').softmax(1, name='prob1'))
...
def detect_face(img, minsize, pnet, rnet, onet, threshold, factor):
    ...
    if numbox > 0:
        ...
        for k in range(0, numbox):
            ...
            tmp[dy[k] - 1:edy[k], dx[k] - 1:edx[k], :] = img[y[k] - 1:ey[k], x[
                k] - 1:ex[k], :]
...
def bulk_detect_face(images, detection_window_size_ratio, pnet, rnet, onet,
        ...
        for index in range(len(outs[0])):
            ...
            if boxes.size > 0 and pick.size > 0:
                ...
                images_with_boxes[image_index]['total_boxes'] = np.append(
                    images_with_boxes[image_index]['total_boxes'], boxes,
                    axis=0)
    ...
    for index, image_obj in enumerate(images_with_boxes):
        ...
        if numbox > 0:
            ...
            regh = image_obj['total_boxes'][:, 3] - image_obj['total_boxes'][:,
                                                                             1]
            qq1 = image_obj['total_boxes'][:, 0] + image_obj[
                'total_boxes'][:, 5] * regw
            qq2 = image_obj['total_boxes'][:, 1] + image_obj[
                'total_boxes'][:, 6] * regh
            qq3 = image_obj['total_boxes'][:, 2] + image_obj[
                'total_boxes'][:, 7] * regw
            qq4 = image_obj['total_boxes'][:, 3] + image_obj[
                'total_boxes'][:, 8] * regh
            image_obj['total_boxes'] = np.transpose(np.vstack(
                [qq1, qq2, qq3, qq4, image_obj['total_boxes'][:, 4]]))
            image_obj['total_boxes'] = rerec(image_obj['total_boxes'].copy())
            image_obj['total_boxes'][:, 0:4] = np.fix(image_obj[
                'total_boxes'][:, 0:4]).astype(np.int32)
    ...
    for index, image_obj in enumerate(images_with_boxes):
        ...
        image_obj['total_boxes'] = np.hstack([image_obj['total_boxes'][ipass[
            0], 0:4].copy(), np.expand_dims(score_per_image[ipass].copy(), 1)])
        ...
        if image_obj['total_boxes'].shape[0] > 0:
            ...
            if numbox > 0:
                tempimg = np.zeros((48, 48, 3, numbox))
                image_obj['total_boxes'] = np.fix(image_obj[
                    'total_boxes']).astype(np.int32)
                dy, edy, dx, edx, y, ey, x, ex, tmpw, tmph = pad(
                    image_obj['total_boxes'].copy(), w, h)
    ...
    for index, image_obj in enumerate(images_with_boxes):
        ...
        points_per_image[0:5, :] = np.tile(w, (
            5, 1)) * points_per_image[0:5, :] + np.tile(
                image_obj['total_boxes'][:, 0], (5, 1)) - 1
        points_per_image[5:10, :] = np.tile(h, (
...
def generateBoundingBox(imap, reg, scale, t):
    ...
    if y.shape[0] == 1:
        ...
        dy2 = np.flipud(dy2)
    score = imap[(y, x)]
    reg = np.transpose(np.vstack([dx1[(y, x)], dy1[(y, x)], dx2[(y, x)], dy2[(
        y, x)]]))
    if reg.size == 0:
        reg = np.empty((0, 3))
