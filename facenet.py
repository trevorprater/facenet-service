...
def read_and_augument_data(image_list,
                           label_list,
                           image_size,
                           batch_size,
                           max_nrof_epochs,
...
def crop(image, random_crop, image_size):
    if image.shape[1] > image_size:
        ...
        if random_crop:
            diff = sz1 - sz2
            (h, v) = (np.random.randint(-diff, diff + 1), np.random.randint(
                -diff, diff + 1))
        else:
            (h, v) = (0, 0)
...
def load_model(model_dir, meta_file, ckpt_file):
    ...
    saver.restore(tf.get_default_session(), os.path.join(model_dir_exp,
                                                         ckpt_file))
...
def calculate_val_far(threshold, dist, actual_issame):
    ...
    false_accept = np.sum(np.logical_and(predict_issame, np.logical_not(
        actual_issame)))
